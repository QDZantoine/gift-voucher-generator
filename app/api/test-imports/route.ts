  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Erreur import Prisma", details: error.message },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Erreur générateur de code", details: error.message },
        { status: 500 }
      );
    }
    ...
    try {
      const { sendEmailWithRetry, generateGiftCardEmailHTML } = await import(
        "@/lib/email"
      );
      ...
      return NextResponse.json(
        { error: "Erreur import email", details: error.message },
        { status: 500 }
      );
    }
    ...
    try {
      const { generateGiftCardPDF } = await import("@/lib/pdf-generator");
      console.log("✅ Générateur PDF importé avec succès");
    } catch (error) {
      console.error("❌ Erreur import PDF:", error);
      return NextResponse.json(
        { error: "Erreur import PDF", details: error.message },
        { status: 500 }
      );
    }
